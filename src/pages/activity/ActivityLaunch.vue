...
const submit = () => {
  baseform.value.validate().then((res: form) => {
    ...
    if (res) {
      const sum = populationLimit.value.reduce((prev, cur) => {
    ...
    <view class="forms">
      <uni-forms ref="baseform" label-position="top" :rules="rules" :modelValue="content" label-width="100%">
        ...
        <uni-forms-item required label="举办时间" name="time">
          <uni-datetime-picker v-model="content.time" @change="" type="datetime" />
        </uni-forms-item>
        ...
        <uni-forms-item label="学院限制" name="colleges">
          <uni-data-checkbox :multiple="true" v-model="selectedColleges" :localdata="colleges" mode="default"
        ...
        <view name="collegePopulation" label="学院报名人数限制">
          <uni-section v-for="(college, index) in selectedColleges" :key="index" :title="`设置${college}的报名人数`" type="line"
            padding>
            <uni-number-box v-model="populationLimit.find(item => item.college === college)!.population"></uni-number-box>
          </uni-section>
        </view>
        ...
        <uni-forms-item required label="活动简介" name="introduction">
          <textarea @blur="" class="text" :maxlength=-1 v-model="content.introduction"></textarea>
        </uni-forms-item>
        <!-- 主讲人简介 -->
        <uni-forms-item required label="主讲人简介" name="sponsorIntroduction">
          <textarea @blur="" class="text" :maxlength=-1 v-model="content.sponsorIntroduction"></textarea>
        </uni-forms-item>
        <view class="btns">
          <button @tap="submit" class="btn" type="primary">提交</button>
          <button @tap="save" class="btn" type="default">保存</button>
